...
## D-022 — Central Shadow uses Registry operation identity and fails closed on contradiction

All newly approved Central Accounting Shadow orchestration must begin at the Central Accounting Registry. A stored `operationKind` that contradicts the operation kind resolved by the Registry from `tx` blocks the entire Shadow run before parity. When a covered historical row has no stored `operationKind`, Shadow may analyze it only through a temporary read-only parity copy whose operation kind is supplied by the Registry; the source row remains unchanged and legacy fallback does not become an identity authority. Historical-only operations remain historical-only and are not promoted into the new-write path. Phase 2 is read-only, does not activate Production Shadow execution, and changes no protected accounting/data surface. See ADR-011.

## D-023 — Downstream outputs must prove parity through the Central Shadow boundary before runtime wiring

Before any read-only Production consumer is switched to the Central path, the accepted Central Shadow result must prove that Registry-approved temporary operation identity produces exactly the same existing Ledger/financial projection, Unified Trial Balance, and EGP Financial Statements as the untouched stored Entry rows. Downstream evidence runs only after Shadow is unblocked and exact. It reuses the existing reporting engines, introduces no new accounting calculation, leaves source rows unchanged, and fails closed on any downstream mismatch. Phase 3 does not activate live Shadow execution, change any writer, or alter protected accounting/data surfaces. See ADR-012.

## D-024 — Unified Trial Balance is the first Central read-only runtime consumer

Phase 4 begins with the Unified Trial Balance only. Phase 3 remains the offline acceptance proof that exact Registry-approved identity leaves downstream outputs unchanged; the interactive Trial Balance must not recalculate the whole Phase 3 Ledger + Trial Balance + Financial Statements evidence chain on each refresh. Its runtime adapter begins at the Central Registry-gated Shadow, requires `status=compared`, complete parity identity and `exactParity=true`, then supplies Registry-approved operation identity only to temporary in-memory Entry copies before calling the existing `buildUnifiedTrialBalance` engine. Stored `Entry.operationKind` is not a fallback authority, and the UI must not silently fall back to its old direct path if the Central gate blocks. This first runtime wiring step changes no writer or protected accounting/data surface and does not authorize deployment. See ADR-013.

## D-025 — General Ledger is the second Central read-only runtime consumer

Phase 4B routes General Ledger details through the same Central Registry-gated Shadow identity boundary used by Phase 4A. One exact Shadow run supplies complete Registry-approved temporary operation identity to all Ledger dimensions and the all-time summary. Existing `getAvailableDimensions`, Balance Engine period calculations, and `buildLedgerReport` remain the calculation authorities; React no longer invokes them directly. Referenced inactive historical accounts may be temporarily activated only on in-memory Shadow copies while original accounts remain inactive and Ledger presentation remains unchanged. The UI must fail closed with no direct legacy fallback. Phase 4B changes no writer or protected accounting/data surface and does not authorize deployment. See ADR-014.

## D-026 — Income, Financial Position, and Equity complete the approved Central read-only runtime consumers

Phase 4C routes the existing EGP Income Statement, monthly Financial Position, and Statement of Changes in Equity through the shared Central Registry-gated Shadow identity boundary. Each adapter requires complete exact Registry-approved identity, preserves referenced inactive accounts only as temporary Shadow compatibility copies, and then calls the unchanged existing reporting engine. Rows after the requested income end date or report cutoff are excluded before Shadow so irrelevant future rows cannot block an earlier report. Financial Position CSV export uses the same Central adapter and may not call the monthly engine directly from React. Equity UI may not directly call `buildEquityStatementEgp` or `computeAccountBalances`; the existing Equity engine and Balance Engine diagnostic execute behind the Central runtime gate. `FinancialStatementsView` remains a presentation-only wrapper around the Central-wired Income and Financial Position child views. Phase 4C changes no writer, report accounting semantics, protected accounting/data surface, or deployment state. See ADR-015.
