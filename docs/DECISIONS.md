...
Phase 4 begins with the Unified Trial Balance only. The Trial Balance UI must enter through a Central read-only runtime adapter that first requires the accepted Phase 3 evidence boundary to be exact and matched. Runtime operation identity comes exclusively from complete Central Shadow parity on temporary in-memory Entry copies; stored `Entry.operationKind` is not a fallback authority. The existing `buildUnifiedTrialBalance` engine remains the calculation authority and is not reimplemented in UI code. If Central evidence or parity identity is incomplete, the runtime path fails closed and the UI must not silently fall back to the previous direct path. This first runtime wiring step changes no writer or protected accounting/data surface and does not authorize deployment. See ADR-013.
