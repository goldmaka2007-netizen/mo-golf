...
## D-026 — EGP Income Statement and Financial Position complete the approved Central read-only runtime consumers

Phase 4C routes the existing EGP Income Statement and monthly Financial Position through the shared Central Registry-gated Shadow identity boundary. The runtime adapters require complete exact Registry-approved identity, preserve referenced inactive accounts only as temporary Shadow compatibility copies, and then call the unchanged `buildFinancialStatementsEgp` or `buildMonthlyFinancialPosition` engines. Rows after the requested income end date or monthly cutoff are excluded before Shadow so irrelevant future rows cannot block an earlier statement. Financial Position CSV export must use the same Central adapter and may not call the monthly engine directly from React. `FinancialStatementsView` remains a presentation-only wrapper around these two Central-wired child views. Phase 4C changes no writer, Financial Statement accounting semantics, protected accounting/data surface, or deployment state. The separate Equity Statement is not rewired by this decision. See ADR-015.
