...
interface SettingsAccountsPanelProps { onOpenGuide: () => void; }

export const SettingsAccountsPanel = React.memo(({ onOpenGuide }: SettingsAccountsPanelProps) => (
  <div className="space-y-4"><div className="bg-[#0e1018] border border-[#1a1e2a] rounded-3xl p-8 text-center space-y-6">
    ...
    <button onClick={onOpenGuide} className="w-full py-4 bg-[#6a8a9e] text-[#080a0f] rounded-2xl font-bold flex items-center justify-center gap-3 shadow-xl shadow-[#6a8a9e11] active:scale-95 transition-all">تعديل شجرة الحسابات <ChevronRight className="w-4 h-4 rotate-180" /></button>
  </div></div>
));
