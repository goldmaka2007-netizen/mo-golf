...
const Top = ({ title, value, gold, silver, tone, net = false }: { title: string; value: number; gold: number; silver: number; tone: string; net?: boolean }) => <div className={`rounded-2xl border p-4 ${tone}`}><b>{title}</b><div className="mt-2 font-mono text-xl font-black">{money(value)}</div>{gold !== 0 && <div className="mt-2 text-xs text-[#c9a84c]">ذهب{net ? ' (صافي)' : ''}: {formatWeight(gold, 3)} جرام عيار 21</div>}{silver !== 0 && <div className="mt-1 text-xs text-slate-300">فضة{net ? ' (صافي)' : ''}: {formatWeight(silver, 3)} جرام</div>}</div>;
