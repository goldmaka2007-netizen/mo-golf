...
export const BalanceSheetView = React.memo(({ entries }: { entries: Entry[] }) => {
  ...
  return <div className="space-y-4 pb-20" dir="rtl">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <details className="space-y-2 rounded-2xl border border-[#1a1e2a] bg-[#0e1018] p-4"><summary className="cursor-pointer font-black text-[#6a9e6a]">الأصول (Assets)</summary><div className="mt-3 space-y-2"><Row label="النقدية" value={data.assets.cash} /><Row label="مخزون الذهب" value={data.assets.goldInventory} /><Row label="مخزون الفضة" value={data.assets.silverInventory} /><Row label="مخزون الملحقات" value={data.assets.accessoriesInventory} /><Row label="الذمم المدينة" value={data.assets.receivables} /><Row label="إجمالي الأصول" value={data.assets.total} total /></div></details>
      <details className="space-y-2 rounded-2xl border border-[#1a1e2a] bg-[#0e1018] p-4"><summary className="cursor-pointer font-black text-[#d08a6a]">الخصوم (Liabilities)</summary><div className="mt-3 space-y-2"><Row label="التزامات تجار الذهب — Book Value" value={data.liabilities.merchantGold} /><Row label="التزامات تجار الفضة — Book Value" value={data.liabilities.merchantSilver} /><Row label="التزامات التجار النقدية / المصنعية" value={data.liabilities.merchantCash} /><Row label="خصوم أخرى" value={data.liabilities.other} /><Row label="إجمالي الخصوم" value={data.liabilities.total} total /></div></details>
      <details className="space-y-2 rounded-2xl border border-[#1a1e2a] bg-[#0e1018] p-4"><summary className="cursor-pointer font-black text-[#c9a84c]">حقوق الملكية (Equity)</summary><div className="mt-3 space-y-2"><Row label="رأس المال" value={data.equity.capital} /><Row label="الأرباح المحتجزة" value={data.equity.retainedEarnings} /><Row label="ربح الفترة الحالية" value={data.equity.currentProfit} /><Row label="إجمالي حقوق الملكية" value={data.equity.total} total /></div></details>
    </div>
    {data.liabilities.merchantDetails.length > 0 && <details className="rounded-2xl border border-[#1a1e2a] bg-[#0e1018] p-4"><summary className="cursor-pointer font-black text-[#d08a6a]">تفاصيل التزامات التجار</summary><div className="mt-3 space-y-2">{data.liabilities.merchantDetails.map(item => <div key={item.id} className="rounded-xl bg-[#080a0f] p-3 text-xs"><div className="flex justify-between gap-2"><b className="break-words">{item.label}</b><span className="font-mono text-[#c9a84c]">{money(item.bookValue + item.cashPayable)} ج.م</span></div><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[#8a8172]">{item.metal === 'gold' && <span>{formatWeight(item.equivalent21Weight, 3)} جم مكافئ عيار 21</span>}{item.metal === 'silver' && <span>{formatWeight(item.silverWeight, 3)} جم فضة</span>}<span>قيمة المعدن: {money(item.bookValue)} ج.م</span><span>مستحق نقدي: {money(item.cashPayable)} ج.م</span></div></div>)}</div></details>}    <details className="space-y-3 rounded-2xl border border-[#1a1e2a] bg-[#0e1018] p-4"><summary className="cursor-pointer font-black">تفاصيل تقييم المخزون</summary>
      <div className="flex flex-wrap justify-between gap-3"><div><p className="text-[10px] text-[#8a8172]">القيمة السوقية والفرق غير المحقق للعرض فقط ولا يدخلان أي إجمالي.</p></div><div className="flex gap-3 text-xs"><label><input type="checkbox" checked={showWeight} onChange={e => setShowWeight(e.target.checked)} /> الوزن</label><label><input type="checkbox" checked={showMarket} onChange={e => setShowMarket(e.target.checked)} /> السوق والفرق</label></div></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-xs"><thead><tr className="border-b border-[#1a1e2a] text-[#8a8172]"><th className="p-2 text-right">الصنف</th>{showWeight && <th className="p-2 text-left">مكافئ 21 / فضة / كمية</th>}<th className="p-2 text-left">Book Value (ج.م)</th>{showMarket && <><th className="p-2 text-left">Market Value (ج.م)</th><th className="p-2 text-left">Unrealized Difference</th></>}</tr></thead><tbody>{data.inventory.map(x => <tr key={`${x.kind}-${x.label}`} className="border-b border-[#1a1e2a]"><td className="p-2 font-bold">{x.label}</td>{showWeight && <td className="p-2 text-left font-mono">{x.weight !== null ? `${formatWeight(x.weight, 3)} جم` : `${formatQuantity(x.quantity ?? 0, 3)} قطعة`}</td>}<td className="p-2 text-left font-mono text-[#c9a84c]">{money(x.bookValue)}</td>{showMarket && <><td className="p-2 text-left font-mono">{x.marketValue === null ? '—' : money(x.marketValue)}</td><td className="p-2 text-left font-mono">{x.unrealizedDifference === null ? '—' : money(x.unrealizedDifference)}</td></>}</tr>)}</tbody></table></div>
    </details>
