...
export const BalanceSheetView = React.memo(({ entries, onOpenLedger }: { entries: Entry[]; onOpenLedger?: (accountId: string) => void }) => {
  ...
  const exportReport = () => {
    ...
    if (latestReport.available === false) { setExportDiagnostic(`تعذر التصدير: ${latestReport.diagnostic.message}`); return; }
