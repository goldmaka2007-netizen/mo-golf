...
export const CanonicalAccountsPanel = React.memo(() => {
  ...
  const runMigration = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'audit_logs', generateId()), { userId: user.uid, action: 'account_id_migration', migrationVersion: migration.migrationVersion, migratedEntries: ready.length, createdAt: new Date().toISOString() });
  ...
  const approveRegistry = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'audit_logs', generateId()), { userId: user.uid, action: 'canonical_registry_approved', accountCount: registry.accounts.length, createdAt: timestamp });
  ...
  const createDefinition = async () => {
    ...
    const manualAccountId = generateId('manual');
    const definition: CanonicalAccountDefinition = {
      id: manualAccountId, entityId: manualAccountId, userId: user.uid,
