...
describe('pricingConfig presentation boundaries', () => {
  ...
  it('keeps Smart Sale mobile UX presentation-only and backed by the captured program price', () => {
    ...
    expect(assistant).toContain('سعر بيع الذهب');
    expect(assistant).toContain('session.gold21PriceSnapshot');
    expect(assistant).toContain('officialGoldKaratPrice(session.gold21PriceSnapshot, 18 / 21)');
    expect(assistant).toContain('officialGoldKaratPrice(session.gold21PriceSnapshot, 24 / 21)');
    expect(assistant).toContain('[18, gold18PriceSnapshot]');
    expect(assistant).toContain('[21, session.gold21PriceSnapshot]');
    expect(assistant).toContain('[24, gold24PriceSnapshot]');
    expect(assistant).toContain("8: 'جنيه ذهب — 8 جم'");
    expect(assistant).toContain("4: 'نصف جنيه — 4 جم'");
    expect(assistant).toContain("2: 'ربع جنيه — 2 جم'");
    expect(assistant).toContain('const coinRows = [8, 4, 2]');
    expect(assistant).not.toContain('ثمن جنيه');
    expect(assistant).not.toContain('1 جم');
    expect(assistant).not.toContain('معلومة مهمة');
