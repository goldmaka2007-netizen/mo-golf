...
import type { HistoricalInventoryOverlayDirective } from './inventoryCostTypes';
...
export const buildMonthlyFinancialPosition = (args: {
...
}): FinancialStatementsEgp & { metalSummary: FinancialPositionMetalSummary } => {
  ...
  const goldAssetWeight = statements.balanceSheet.inventoryCategories.gold.weight ?? 0;
  const silverAssetWeight = statements.balanceSheet.inventoryCategories.silver.weight ?? 0;
  const goldLiabilityWeight = statements.balanceSheet.liabilities.merchantDetails.filter(row => row.metal === 'gold').reduce((sum, row) => sum + row.equivalent21Weight, 0);
  const silverLiabilityWeight = statements.balanceSheet.liabilities.merchantDetails.filter(row => row.metal === 'silver').reduce((sum, row) => sum + row.silverWeight, 0);
  return { ...statements, metalSummary: { goldAssetWeight, silverAssetWeight, goldLiabilityWeight, silverLiabilityWeight, netGoldWeight: goldAssetWeight - goldLiabilityWeight, netSilverWeight: silverAssetWeight - silverLiabilityWeight } };
};
