...
import { getEntryArabicWeight, parseCash, resolveMerchantMetalOperationSemantic, resolveOperationKind } from './engine';
...
export const buildCanonicalPosting = (entry: Entry, registry: AccountRegistry): CanonicalPostingResult => {
  ...
  const merchantSemantic = resolveMerchantMetalOperationSemantic(entry, debit, credit);
  ...
  return {
    ...
    dimensions, values,
    inventoryImpact: merchantSemantic.kind === 'receipt' || merchantSemantic.kind === 'weight_settlement'
      ? true
      : merchantSemantic.kind === 'cash_settlement' || merchantSemantic.kind === 'merchant_transfer' || merchantSemantic.kind === 'opening'
        ? false
