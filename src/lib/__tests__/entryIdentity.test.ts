...
const base = (overrides: Partial<Entry> = {}): Entry => ({ seq: 1, tx: saleTx, debit: 'cash', credit: 'gold-ring', date: '2026-07-22', cash: '1000', weight: '1', arabicWeight: '1', count: '0', notes: '', userId: 'u', ...overrides });
...
describe('resolveEntryIdentity', () => {
  ...
  it('progressively upgrades an old entry without removing historical fields', () => {
    const oldEntry = base({ id: 'legacy-entry', notes: 'historical note' });
    expect(resolveEntryIdentity(oldEntry, accounts)).toMatchObject({ ok: true, value: { operationKind: 'sale', debitAccountId: 'cash-id', creditAccountId: 'gold-ring-id' } });
    expect(oldEntry).toMatchObject({ id: 'legacy-entry', debit: 'cash', credit: 'gold-ring', notes: 'historical note' });
  });
  it('rejects a write when a selected account cannot be resolved', () => {
    expect(resolveEntryIdentity(base({ debit: 'missing' }), accounts)).toEqual(expect.objectContaining({ ok: false, message: expect.stringContaining('\u062A\u0639\u0630\u0631 \u062A\u062D\u062F\u064A\u062F \u0645\u0639\u0631\u0641 \u0627\u0644\u062D\u0633\u0627\u0628 \u0627\u0644\u0645\u062F\u064A\u0646') }));
  });
});