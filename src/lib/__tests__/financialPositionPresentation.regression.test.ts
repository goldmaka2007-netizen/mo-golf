...
const accounts: Account[] = [
  ...
  { id: 'fixed', name: 'Asset A', mainType: 'assets', subType: 'أصول ثابتة', canonicalMainType: 'assets', balanceNature: 'cash', type: 'other', userId: 'u' },
...
describe('Financial Position presentation regression', () => {
  it('classifies registry-resolved fixed assets separately when source canonical metadata is absent', () => {
    const report = buildFinancialStatementsEgp([
      ...
      entry({ id: 'fixed', debit: 'Asset A', debitAccountId: 'fixed', credit: 'Cash', creditAccountId: 'cash', cash: '200' }),
    ...
    expect(report.balanceSheet.assets.ordinaryReceivableDetails).not.toContainEqual(expect.objectContaining({ accountId: 'fixed' }));
